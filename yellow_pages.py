...
url = f"https://www.yellowpages.com.au/search/listings?clue=automotive+repair&locationClue=Greater+Melbourne%2C+VIC&pageNumber=2"
def get_data():
    global header_added
    driver.get(url)
    ...
    for rest, cat, ph, st, lo in zip(rests, cats, phones, st_addr, locality):
        dict1 = {'Firm Name': rest.text, "Category": cat.text, "Phone Number": ph.text, "Street Address": st.text,
                 "Locality": lo.text}
        with open(f'Yellow_Pages_NewYork.csv', 'a+', encoding='utf-8-sig') as f:
...
while True:
    try:
        i += 1
        url = f"https://www.yellowpages.com.au/search/listings?clue=automotive+repair&locationClue=Greater+Melbourne%2C+VIC&pageNumber={i}"
        get_data()
